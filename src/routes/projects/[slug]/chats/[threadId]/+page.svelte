...
<script lang="ts">
	...
	const slug = $derived(data.project.frontmatter.slug);
	...
	onMount(() => {
		if (window.matchMedia('(min-width: 768px)').matches) {
			goto(`/projects/${slug}`, { replaceState: true });
		}
	});
	...
	async function sendCurrentMessage(): Promise<void> {
		...
		try {
			...
			const res = await fetch(`/api/projects/${slug}/threads/${threadId}/chat/stream`, {
				method: 'POST',
	...
	async function sendCurrentMessageAtomic(
		...
		try {
			const result = await withRetry(async () => {
				const res = await fetch(`/api/projects/${slug}/threads/${threadId}/chat`, {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ message: text })
				});
	...
	async function reconcileMessages(threadId: string): Promise<void> {
		try {
			const res = await fetch(`/api/projects/${slug}/threads/${threadId}/messages`);
			if (!res.ok) return;
	...
	async function commitRename(): Promise<void> {
		...
		try {
			const result = await withRetry(async () => {
				const res = await fetch(`/api/projects/${slug}/threads/${localThread.id}`, {
					method: 'PATCH',
					headers: { 'Content-Type': 'application/json' },
